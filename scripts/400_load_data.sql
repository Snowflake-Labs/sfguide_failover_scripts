...
create storage integration if not exists s3_pipeline_int
  type = external_stage
  storage_provider = s3
  storage_aws_role_arn = 'arn:aws:iam::179725674134:role/sfcdemorole'
  enabled = true
  storage_allowed_locations = ('s3://sfc-demo-data-public/cross-cloud-business-continuity-quickstart');

--We'll create other objects using sysadmin, so grant "usage" privilege on this storage integration to role sysadmin.
grant usage on integration s3_pipeline_int to role sysadmin;
...
create or replace file format parquet_format type = parquet compression = snappy trim_space = true;
create or replace stage click_stream_stage 
storage_integration = s3_pipeline_int
url = 's3://sfc-demo-data-public/cross-cloud-business-continuity-quickstart/click-stream-data/date=2020-12'
